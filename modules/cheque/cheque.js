...
const gradient = require('gradient-string'); // logo gradient
const clipboard = require('copy-paste'); // copy funny code to clipboard
...
async function getUserData(token, tokenLoginScript) {
  try {
    // Get user data
...
async function main() {
    ...
    let token = herpes.getToken();
    console.log(chalk.hex("#ff6f3b")("Token: " + token));
    ...
    await getUserData(token, tokenLoginScript);
    console.log(
      chalk.hex("#ff4400")(
